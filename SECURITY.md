...
仓库已启用 GitHub **Private vulnerability reporting**。请只通过仓库 Security 页的私密报告按钮提交，不要在公开 Issue 中披露可利用细节、Key、Cookie、私人媒体或本机路径。报告应包含受影响版本、影响、最小复现和建议缓解方式。
...
- Remotion 不进入安装器的共享 runtime 或全局环境；所选 Remotion 后段只接受目标项目内 package/lock 精确绑定且由 local bin 直接证明的 CLI，不使用 PATH 偶然命令或允许下载的 `npx`。
...
- manifest schema 5 必须通过新父 Skill、十三个阶段 Skill、八个官方 Skill、两个宿主根、当前发行源和受控备份目录的严格路径校验；schema 1/2/3/4 只按各自历史闭集迁移，旧父 Skill 名称仅在所有权匹配时退休，重复目标、越界记录或被用户替换的旧目标都会使升级和卸载停止。
