...
import { computeRuntimePlanIdentity, validateRuntimePlan } from './validate-runtime-plan.mjs';
...
function actionPlan(selection, mode, warnings) {
  const plan = {
    schemaVersion: '1.0.0', status: 'action-required', planningMode: mode,
    selection: {
    ...
    frozenMediaContractVersion: null, shots: [], blocks: [], warnings: [...new Set(warnings)].sort(),
    identity: '',
  };
...
export async function planRuntime({ recipesDirectory, selectionFile, matrixFile = defaultMatrix, remotionIndexFile = defaultRemotionIndex }) {
  const [recipes, selection, matrix, remotionIndex] = await Promise.all([
  ...
  if (conflicts.length) {
    const plan = actionPlan(selection, mode, [...warnings, ...conflicts]);
    await validateRuntimePlan(plan);
    return plan;
  }
  const resultingRoute = backends.length === 2 ? 'hybrid' : backends[0];
  const plan = {
    schemaVersion: '1.0.0', status: 'planned', planningMode: mode,
    selection: {
      schemaVersion: selection.schemaVersion,
      selectedRuntime: selection.selectedRuntime,
      selectionSource: selection.selectionSource,
    ...
    shots, blocks: buildBlocks(shots), warnings: [...new Set(warnings)].sort(), identity: '',
  };
  plan.identity = computeRuntimePlanIdentity(plan);
  await validateRuntimePlan(plan);
  return plan;
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--recipes', '--selection', '--matrix', '--remotion-index'].includes(name)) throw new Error(`unknown argument ${name}`);
    const value = argv[index + 1];
    if (!value) throw new Error(`${name} requires a path`);
    options[name.slice(2)] = path.resolve(value);
    index += 1;
  }
  if (!options.recipes || !options.selection) throw new Error('--recipes and --selection are required');
  return {
