...
- Integer SRT milliseconds cover zero through final cue end continuously and
  every cue belongs to one semantic shot.
- `narrative-envelope.json` stores whole-film proposition, sections, context,
  and terminology once.
- `visual-system.json` stores the concrete visual world, palette/typography
...
- Non-empty needs followed only suitable routes: user material, authorized
  ...
  source rather than stock substitution.
- Any Pexels search was real, credential-scoped, need-bound, and preserved
  selection/rejection facts. A missing key blocked only a Pexels-routed need.
- Every selected file exists locally and records source, rights, objective
