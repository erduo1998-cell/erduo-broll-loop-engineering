...
export function detectImageGeneration(capability) { if (!capability || capability.available !== true || typeof capability.adapter_id !== 'string' || !/^[a-z0-9-]{2,64}$/u.test(capability.adapter_id) || !Array.isArray(capability.image_formats) || !capability.image_formats.includes('png')) return { available: false, route: 'pexels' }; return { available: true, adapter_id: capability.adapter_id, formats: [...new Set(capability.image_formats)].sort() }; }
...
export async function generateAndFreezeImage(projectRoot, brief, design, target, capability, adapter, { probeMedia, fsImpl = fs } = {}) {
  const detected = detectImageGeneration(capability); if (!detected.available) return { route: 'fallback', next_route: 'pexels', reason_code: 'IMAGE_GENERATION_UNAVAILABLE' };
