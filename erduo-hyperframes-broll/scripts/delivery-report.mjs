...
const ROUTES = new Set(['user-media', 'pexels', 'image-generation', 'hyperframes-native']);
const GATES = new Set(['input-time', 'shot-design', 'asset-integrity', 'delivery-media', 'frame-visibility']);
...
export function validateDeliverySummary(value) {
  ...
  exact(value.master, ['path', 'duration_ms']); exact(value.shots, ['directory', 'count']);
  if (value.schema_version !== 1 || !publicPath(value.master.path) || !Number.isSafeInteger(value.master.duration_ms) || value.master.duration_ms <= 0 || !publicPath(value.shots.directory) || !Number.isSafeInteger(value.shots.count) || value.shots.count < 1 || !Array.isArray(value.routes) || !value.routes.length || !Array.isArray(value.passed_gates) || !Array.isArray(value.limitations)) fail('invalid_delivery', 'Delivery summary is invalid.');
...
const routeText = { 'user-media': '用户素材', pexels: 'Pexels', 'image-generation': '生图', 'hyperframes-native': 'HyperFrames 原生图形' };
...
export function buildDeliveryReport(summary) {
  ...
  const core = { schema_version: 1, master_path: summary.master.path, shots_directory: summary.shots.directory, shot_count: summary.shots.count, duration_ms: summary.master.duration_ms, routes, passed_gates: gates, limitations };
  const text = [`已完成候选 B-roll 交付。`, `Master：${core.master_path}`, `逐镜素材：${core.shots_directory}（${core.shot_count} 镜）`, `总时长：${durationText(core.duration_ms)}`, `素材路线：${routes.map((item) => routeText[item]).join('、')}`, `已通过：${gates.join('、')}`, ...(limitations.length ? [`真实限制：${limitations.map((item) => limitationText[item]).join(' ')}`] : [])].join('\n');
